  ...
  function($scope, $rootScope, $timeout, $log, $modal, profileService, go, notification, isCordova, gettext, gettextCatalog, animationService) {
    ...
    self.shareSecret = function(secret) {
      if (isCordova) {
        ...
        var message = gettextCatalog.getString('Join my DigiByte Gaming wallet. Here is the invitation code: {{secret}} You can download DigiByte Gaming for your phone or desktop at https://digibyte.co', {secret: secret});
        window.plugins.socialsharing.share(message, gettextCatalog.getString('Invitation to share a DigiByte Gaming Wallet'), null, null);
      }
    };
